...
*/
	...
	function userExistDb($user){
		

	 	$oneUser = getOneUserDb($user);
	...
	function samePassDb($user,$pass){

	 	$oneUser = getOneUserDb($user);

	 	if(password_verify($oneUser['pass'], $pass)){
			return true;
	 	}
		return false;
	}

	function emailExistDb($email){
		$oneUser = getOneUserDb($email);
	...
	function validateLogin(){
		...
		} elseif (!userExistDb($user)) {
			$errores['user'] = "Este usuario no existe";
		} else {

			/*$theUser = getOneUser($user);*/

			$theUser = getOneUserDb($user);

			if (!password_verify($password, $theUser['pass']) ) {
